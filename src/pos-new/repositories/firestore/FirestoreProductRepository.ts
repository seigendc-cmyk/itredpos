...
export function createFirestoreProductRepository(): ProductRepository {
  return {
    ...
    async listProducts(context: RepositoryOperationContext): Promise<{ success: boolean; records: SharedProductRecord[]; errorCode?: string; errorMessage?: string }> {
      try {
        validateRepositoryOperationContext(context);
      } catch (validationError) {
        const message = validationError instanceof Error ? validationError.message : 'Invalid repository operation context.';
      ...
      try {
        const q = query(collection(db, firestorePaths.productMaster(context.vendorId)), where('vendorId', '==', context.vendorId));
