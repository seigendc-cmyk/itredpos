...
import { getOpenCustomerDebitEntries } from './customerAccountService';
import type { PosSession } from '../types';

export interface CustomerAgeingBreakdown {
  customerId: string;
...
function riskLevel(overdueDays: number, totalOutstanding: number): CustomerAgeingBreakdown['riskLevel'] {
  if (overdueDays > 90 || totalOutstanding >= 1000) return 'Critical';
  if (overdueDays > 60 || totalOutstanding >= 500) return 'High';
...
): CustomerAgeingBreakdown {
  const context = assertCanonicalCustomerContext(session);
  const openEntries = getOpenCustomerDebitEntries(context.vendorId, customerId);
  const result: CustomerAgeingBreakdown = {
    customerId,
    totalOutstanding: 0,
    current: 0,
    days30: 0,
