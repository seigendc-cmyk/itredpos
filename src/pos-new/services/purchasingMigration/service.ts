...
export async function executePurchasingMigration(preview: PurchasingMigrationPreview, approval: PurchasingMigrationApproval, writer: CanonicalMigrationWriter, migratedBy: string, previous: PurchasingMigrationRecordResult[] = [], batchSize = 50, onDiagnostic?: (diagnostic: PurchasingMigrationDiagnostic) => void): Promise<PurchasingMigrationRecordResult[]> {
  ...
  for (const record of sorted.slice(0, batchSize)) {
    ...
    try { await writer.migrate(record, { legacySourceType: record.legacySourceType, legacyRecordId: record.legacyRecordId, legacyParentId: record.legacyParentId, sourceFingerprint: record.sourceFingerprint!, migrationRunId: preview.migrationRunId, migratedAt, migratedBy, migrationVersion: approval.migrationVersion, destinationId }); const result = { recordId: record.legacyRecordId, recordType: record.recordType, status: 'migrated' as const, sourceFingerprint: record.sourceFingerprint!, destinationId, migratedAt, retryable: false, attemptNumber }; results.push(result); emit(result); }
