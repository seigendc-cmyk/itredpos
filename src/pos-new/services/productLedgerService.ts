...
const LEDGER_MOVEMENT_TYPE_MAP: Record<InventoryMovementType, string> = {
  ...
  WAREHOUSE_TRANSFER_IN: 'Warehouse Transfer In',
  WAREHOUSE_TRANSFER_OUT: 'Warehouse Transfer Out',
  RAW_TO_FINISHED_OUT: 'Transformation Input Consumption',
  RAW_TO_FINISHED_IN: 'Transformation Output Production',
...
const DISPLAY_TO_MOVEMENT_TYPE: Record<string, InventoryMovementType | InventoryMovementType[]> = {
  ...
  'Stocktake Gain': 'STOCKTAKE_GAIN',
  'Stocktake Loss': 'STOCKTAKE_LOSS',
  'Transfer In': ['TRANSFER_IN', 'BRANCH_TRANSFER_IN', 'WAREHOUSE_TRANSFER_IN'],
  'Transfer Out': ['TRANSFER_OUT', 'BRANCH_TRANSFER_OUT', 'WAREHOUSE_TRANSFER_OUT'],
  'Branch Transfer In': 'BRANCH_TRANSFER_IN',
  'Branch Transfer Out': 'BRANCH_TRANSFER_OUT',
  'Warehouse Transfer In': 'WAREHOUSE_TRANSFER_IN',
  'Warehouse Transfer Out': 'WAREHOUSE_TRANSFER_OUT',
  'Transformation Input Consumption': 'RAW_TO_FINISHED_OUT',
  'Transformation Output Production': 'RAW_TO_FINISHED_IN',
...
export function getLedgerSummaryFromMovements(entries: InventoryMovement[]): ProductLedgerSummary {
  ...
  return {
    ...
    transferMovements: entries.filter(
      (entry) => entry.movementType === 'TRANSFER_IN' || entry.movementType === 'TRANSFER_OUT' || entry.movementType === 'BRANCH_TRANSFER_IN' || entry.movementType === 'BRANCH_TRANSFER_OUT' || entry.movementType === 'WAREHOUSE_TRANSFER_IN' || entry.movementType === 'WAREHOUSE_TRANSFER_OUT'
    ).length,
