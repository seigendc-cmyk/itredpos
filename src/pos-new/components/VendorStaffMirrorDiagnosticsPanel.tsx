...
export default function VendorStaffMirrorDiagnosticsPanel() {
  ...
  const hasIdentity = Boolean(vendorId && uid);
  ...
  const repairOwner = useCallback(async () => {
    ...
    setMessage(null);
    try {
      setMirror(await mirrorOwnerAsBusinessUser(vendorId, uid, ownerEmail));
    ...
    <div className="space-y-4">
      ...
      {!hasIdentity && (
        <div className="p-3 bg-slate-950 border border-slate-800 text-amber-400 font-mono text-[10px] uppercase">
          EMPTY STATE: vendorId or Firebase uid is missing. Sign in with a Google session and open a vendor to enable diagnostics.
        </div>
      )}
